 ...
 * (case-insensitive: `"v1"` or `"v3"`). There is NO silent default: if the
 * variable is unset, blank, or holds an unrecognized value, this THROWS a clear
 * Error. This mirrors the .NET resolver's throw-on-missing semantics so neither
 * SDK can silently fall back to v1.
 *
 * @param env The environment to read from (defaults to `process.env`); injectable for tests.
 * @throws {Error} when `DATASOURCE_FORMAT` is unset/blank or not one of `v1`/`v3`.
 */
export function resolveDataSourceFormat(
  ...
  if (raw === undefined || raw.trim().length === 0) {
    throw new Error(
      `${DATASOURCE_FORMAT_ENV} is not set. It must be "v1" or "v3" — there is no default.`
    );
  }

  const normalized = raw.trim().toLowerCase();
  if (normalized === 'v1' || normalized === 'v3') {
    return normalized;
  }
